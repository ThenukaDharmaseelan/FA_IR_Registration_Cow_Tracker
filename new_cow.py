...
# ── Overlap composites ────────────────────────────────────────────────────────
def make_overlap_image(moving_img, warped_img):
    fov = get_fov_mask(moving_img)
    def norm(img):
...
def make_overlap_vessels(moving_v, warped_v, moving_img):
    fov = get_fov_mask(moving_img)
    H, W = moving_v.shape
    out = np.zeros((H, W, 3), dtype=np.uint8)
    out[..., 0] = (warped_v * 255).astype(np.uint8)
    out[..., 1] = (moving_v  * 255).astype(np.uint8)
...
def make_row_grid(fixed_img, fixed_vessel, moving_img, moving_vessel,
    ...
    ov_img    = make_overlap_image(moving_img, warped_img)
    ov_vessel = make_overlap_vessels(moving_vessel, warped_vessel, moving_img)
    panels = [
...
def save_summary_page(out_dir, rows, mean_before=None, mean_after=None):
    ...
    for r in rows:
        ...
        padded.append(r)
    sep    = np.full((ROW_SEP_H, max_w, 3), (100, 100, 100), dtype=np.uint8)
    BANNER_H = 50
    ...
    cv2.putText(banner, banner_txt, (10, 34), cv2.FONT_HERSHEY_SIMPLEX,
                0.6, (255, 220, 60), 1, cv2.LINE_AA)
    parts = [banner]
    for row in padded:
        parts.append(sep)
        parts.append(row)
...
def main():
    ...
    parser.add_argument("--output_dir",        default="new_results_reverse")
    ...
    if all_rows:
        ...
        save_summary_page(out_dir, all_rows, mean_before=mean_before, mean_after=mean_after)

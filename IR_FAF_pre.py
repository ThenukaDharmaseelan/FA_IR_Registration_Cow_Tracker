...
from preprocess import preprocess_image          # ← shared preprocessing
...
# ── Vessel enrichment ─────────────────────────────────────────────────────────
def enrich_vessel(vessel, prep_img=None):
    """
    3-channel input for CoWTracker from a vessel mask.
    If prep_img (preprocessed RGB) is provided, its grayscale replaces
    the plain dilated channel — this is where preprocessing actually matters.
    """
    ...
    if prep_img is not None:
        ...
        gray = cv2.bitwise_and(gray, gray, mask=dilated)
        ch0  = gray
    else:
        ch0  = dilated

    return np.stack([ch0, dist, blurred], axis=-1)


# ── CoWTracker ────────────────────────────────────────────────────────────────
def run_cowtracker(model, fixed_vessel, moving_vessel,
                   prep_fixed=None, prep_moving=None):
    video = np.stack([enrich_vessel(fixed_vessel,  prep_fixed),
                      enrich_vessel(moving_vessel, prep_moving)], axis=0)
...
# ── Summary pages (paginated, 20 rows per page → FAF_IR/) ────────────────────
...
def main():
    parser = argparse.ArgumentParser(
        description="CoWTracker Retinal Registration — dense warp (IR->FAF) with preprocessing")
    parser.add_argument("--csv",               required=True)
    parser.add_argument("--output_dir",        default="IR_FAF_Pre_outputs")
    ...
    parser.add_argument("--tile_grid",    type=int,   default=16,
    ...
    print("=" * 60)
    print("CoWTracker Retinal Registration  [IR->FAF, dense flow]")
    print("=" * 60)
    print(f"Device       : {DEVICE}")
    print(f"Size         : {args.height} x {args.width}")
    print(f"Rows/page    : {args.rows_per_page}")
    print(f"Preprocessing: CLAHE(clip={args.clahe_clip}, tile={args.tile_grid}x{args.tile_grid})"
          f" -> GaussianBlur(k={args.blur_ksize}, s={args.blur_sigma})")
    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            # ── Preprocessing (for CoWTracker input only) ─────────────────────
            tile = (args.tile_grid, args.tile_grid)
            prep_fixed  = preprocess_image(fixed_img,
                                           clip_limit=args.clahe_clip,
                                           tile_grid=tile,
                                           blur_ksize=args.blur_ksize,
                                           blur_sigma=args.blur_sigma)
            prep_moving = preprocess_image(moving_img,
                                           clip_limit=args.clahe_clip,
                                           tile_grid=tile,
                                           blur_ksize=args.blur_ksize,
                                           blur_sigma=args.blur_sigma)

            # ── CoWTracker on vessel masks (preprocessing improves vessel SNR) -
            tracks, vis, conf = run_cowtracker(model, fixed_vessel, moving_vessel,
                                           prep_fixed=prep_fixed, prep_moving=prep_moving)
            ...
            # ── Warp original images (not preprocessed) ───────────────────────
    ...
    print(f"  Summary pages   : {out_dir.resolve()}/IR_FAF_pre/")
    print("=" * 60)

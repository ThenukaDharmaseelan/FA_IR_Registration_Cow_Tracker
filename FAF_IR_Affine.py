...
# # # ── Hard-reject thresholds (degenerate fit detection) ─────────────────────────
# # REJECT_ABS_ROT   = 90.0    # degrees  — |angle| > 90 is almost certainly wrong
# # REJECT_SCALE_MAX = 3.0     # sx or sy > 3.0 → catastrophic scale
# # REJECT_SCALE_MIN = 0.2     # sy < 0.2 → near-collapse
# # REJECT_NEGATIVE_SCALE = True  # sy < 0 means reflection — always reject
...
# #     """
# #     Estimate a 2x3 affine matrix from CoWTracker correspondences using
# #     RANSAC-based cv2.estimateAffine2D.

# #     Returns
# #     -------
# #     M            : np.ndarray (2, 3) or None
# #     decomp       : dict (tx, ty, angle_deg, sx, sy, shear) or None
# #     meta         : dict (n_candidates, n_inliers, inlier_ratio, low_conf_fallback)
# #     reject_reason: str or None  — set if hard-rejected before returning
# #     """
...
# #                   f"cannot fit affine (need at least 4)")
...
# #     print(f"  Fitting affine from {n_candidates} confident correspondences "
# #           f"(conf>{conf_thresh:.2f}){' [fallback threshold]' if low_conf_fallback else ''}")
...
# #     print(f"  RANSAC inliers: {n_inliers} / {n_candidates}  "
...
# #     print(f"  Decomposition → tx={decomp['tx']:.2f}px  ty={decomp['ty']:.2f}px  "
# #           f"rot={decomp['angle_deg']:.2f}°  "
# #           f"sx={decomp['sx']:.4f}  sy={decomp['sy']:.4f}  "
# #           f"shear={decomp['shear']:.4f}")

# #     # ── Hard-reject degenerate fits ───────────────────────────────────────────
...
# #     """
# #     Returns a rejection reason string if the affine is clearly a bad fit,
...
# #     """
# #     Decompose a 2x3 affine matrix M into:
# #         translation  (tx, ty)
# #         rotation     (angle in degrees, via SVD)
# #         scale        (sx, sy — singular values)
# #         shear        (residual skew after removing rotation & scale)

# #     The decomposition follows:
# #         A  = M[:2, :2]
# #         A  = U @ diag(S) @ Vt     (SVD)
# #         R  = U @ Vt               (pure rotation, det = +1 enforced)
# #         sx, sy = S                 (anisotropic scale)
# #         shear  = off-diagonal of  R^T @ A / diag(S)
# #     """
...
# #     # Enforce proper rotation (det = +1); flip if reflection
...
# #     # Shear: residual off-diagonal of scale-normalised matrix R^T @ A
# #     A_norm = R.T @ A
...
# # # ── Sanity flagging (soft warnings — does NOT block warping) ──────────────────
...
# #                 max_shear=0.25):
# #     """
# #     Return a list of warning strings if the affine parameters look suspicious.
# #     These are soft flags only — degenerate hard-rejection is handled separately.
# #     An empty list means the transform passes all checks.
# #     """
# #     flags = []
...
# #     Sidebar shows pair index, Dice scores, affine decomposition, and flags.
# #     """
# #     GAP   = 6
# #     H     = fixed_img.shape[0]
...
# #                  pair_index=None, decomp=None, flags=None, meta=None):
...
# # # ── Summary pages (20 rows per page) ─────────────────────────────────────────
...
# #             f"{min((p + 1) * rows_per_page, len(rows))}"
# #             f" / {len(rows)}"
# #         )
# #         banner_txt = (
# #             f"CoWTracker Retinal Registration (Affine)  |  "
# #             f"Page {page_label}  |  {pairs_range}"
# #         )
# #         if mean_before is not None and mean_after is not None:
# #             banner_txt += (
# #                 f"  |  Mean Dice before: {mean_before:.4f}  "
...
# #                     cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 220, 60), 1, cv2.LINE_AA)
...
# # # ── Main ──────────────────────────────────────────────────────────────────────
# # def main():
# #     parser = argparse.ArgumentParser(description="CoWTracker Retinal Registration (Affine)")
...
# #     # Soft flagging thresholds (relaxed for cross-modal FA/IR)
...
# #     print("=" * 60)
# #     print("CoWTracker Retinal Image Registration  [Affine Baseline]")
# #     print("=" * 60)
...
# #           f"scale [{args.scale_lo},{args.scale_hi}]  "
# #           f"shear>{args.max_shear}")
# #     print(f"Hard reject: |rot|>{REJECT_ABS_ROT}°  "
# #           f"|scale|>{REJECT_SCALE_MAX} or <{REJECT_SCALE_MIN}  "
# #           f"negative_scale={REJECT_NEGATIVE_SCALE}  "
...
# #             # ── Hard-rejected (degenerate or too few points) ──────────────────
# #             if M is None:
# #                 print(f"  Skipping — {reject_reason}")
# #                 results.append(dict(
# #                     name=name, moving=moving_path, fixed=fixed_path,
# #                     status="degenerate", error=reject_reason,
# #                 ))
# #                 continue
...
# #             warped_img_bgr = warp_with_affine(
...
# #     regressed  = ok[ok["dice_regressed"] == True]   if "dice_regressed"   in ok.columns else pd.DataFrame()
...
# #     print(f"  File errors     : {len(results_df[results_df['status']=='error'])}")
# #     if len(ok):
# #         print(f"  Flagged (soft)  : {len(flagged)}")
# #         print(f"  Regressed       : {len(regressed)}  (dice got worse)")
# #         print(f"  Low-conf fallbk : {len(fallback)}")
...
# #         print(f"  Mean translation: tx={ok['tx'].mean():.1f}px  ty={ok['ty'].mean():.1f}px")
...
#                         help="Apply CLAHE + Gaussian blur preprocessing (default: on)")
...
#                         help="Gaussian blur sigma (default: 1.2)")
#     args = parser.parse_args()
...
#           + (f"  (CLAHE clip={args.clahe_clip}, tile={args.clahe_tile}x{args.clahe_tile},"
#              f" blur ksize={args.blur_ksize}, sigma={args.blur_sigma})"
#              if args.preprocess else ""))
#     print(f"Soft flags : translation>{args.max_translation}px  "
#           f"rotation>{args.max_rotation}°  "
#           f"scale [{args.scale_lo},{args.scale_hi}]  "
#           f"shear>{args.max_shear}")
